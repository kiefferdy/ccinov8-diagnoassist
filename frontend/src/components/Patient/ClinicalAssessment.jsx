...
import { MessageSquare, ChevronRight, ChevronLeft, Plus, Loader } from 'lucide-react';
    ...
    <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-4">
            {patientData.chiefComplaintDetails.map((detail, index) => (
              <div key={index} className="border-l-4 border-blue-500 pl-4">
                <p className="font-medium text-gray-900 mb-1">{detail.question}</p>
                <p className="text-gray-700">{detail.answer}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {!currentQuestion && !isLoading && patientData.chiefComplaintDetails.length > 0 && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-6 mb-6">
          <p className="text-green-800 font-medium">
            ✓ Initial assessment complete. You've gathered {patientData.chiefComplaintDetails.length} responses.
          </p>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        ...
        >
          ...
          <ChevronRight className={`w-5 h-5 text-gray-400 transform transition-transform ${showAdditionalNotes ? 'rotate-90' : ''}`} />
        </button>
